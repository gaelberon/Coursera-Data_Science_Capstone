...
```{r global_data_loading_and_analysis, cache = TRUE}
...
```{r display_global_analysis, cache = TRUE, results = 'asis'}
...
```{r global_data_loading, cache = TRUE}
...
```{r training_dataset_quick_view, cache = TRUE}
...
```{r training_dataset_after_cleaning, cache = TRUE}
...
```{r datasets_cleaning, cache = TRUE}
...
```{r display_datasets_analysis, cache = TRUE, results = 'asis'}
...
```{r dataset_analysis_1Gram, cache = TRUE}
...
```{r display_train1GramSummary, cache = TRUE, results = 'asis'}
...
```{r display_train1GramNoStopWordsSummary, cache = TRUE, results = 'asis'}
...
```{r display_wordcloud_and_histogram_train1Gram, cache = TRUE, fig.height = 4, fig.width = 10, fig.align = 'center'}
...
```{r display_wordcloud_and_histogram_train1GramNoStopWords, cache = TRUE, fig.height = 4, fig.width = 10, fig.align = 'center'}
...
```{r dataset_analysis_2Gram, cache = TRUE}
...
```{r display_wordcloud_and_histogram_train2Gram, cache = TRUE, fig.height = 4, fig.width = 10, fig.align = 'center'}
...
```{r display_wordcloud_and_histogram_train2GramNoStopWords, cache = TRUE, fig.height = 4, fig.width = 10, fig.align = 'center'}
...
```{r dataset_analysis_3Gram, cache = TRUE}
...
```{r display_wordcloud_and_histogram_train3Gram, cache = TRUE, fig.height = 4, fig.width = 10, fig.align = 'center'}
...
```{r display_wordcloud_and_histogram_train3GramNoStopWords, cache = TRUE, fig.height = 4, fig.width = 10, fig.align = 'center'}
...
### 4-gram sequence (quadrigram)

```{r dataset_analysis_4Gram, cache = TRUE}
# PROCEEDING WITH GETTING NGRAM (4) WITH ENGLISH STOP WORDS
library(ngram)
start_time <- proc.time()
train4Gram <- getNgram(trainCorpus, n = 4)
stop_time <- proc.time()
proc_time_per_treatment <- set_proc_time_per_treatment(
        proc_time_per_treatment,
        "Training data set - Call to getNgram function for 4 gram with English stop words",
        start_time,
        stop_time)

# PROCEEDING WITH GETTING NGRAM (4) WITHOUT ENGLISH STOP WORDS
start_time <- proc.time()
train4GramNoStopWords <- getNgram(trainCorpusNoStopWords, n = 4)
stop_time <- proc.time()
proc_time_per_treatment <- set_proc_time_per_treatment(
        proc_time_per_treatment,
        "Training data set - Call to getNgram function for 4 gram without English stop words",
        start_time,
        stop_time)
```
...
## Create the ngrams + frequencies data sets

```{r text_prediction_functions, cache = TRUE}
remove_stop_words_from_ngrams <- function(ngrams) {
        #print("last words:")
        #print(word(as.character(result$ngrams),-1))
        #print(sub(pattern = '^.* ([[:alnum:]]+)$', replacement = '\\1', x = result$ngrams))
        #print(sub(pattern = '.* (\\w+)$', replacement = '\\1', x = result$ngrams))
        #print(stri_extract_last_words(ngrams$ngrams))
        without_stop_words <- !(stri_extract_last_words(ngrams$ngrams) %in% stopwords("en"))
        #print(without_stop_words)
        ngrams <- ngrams[without_stop_words, ]
        return(ngrams)
}

look_for_unigram_from_ngrams <- function(unigram, from_bigrams) {
        pattern <- paste0("^", tolower(unigram), " ")
        result_idx <- grep(pattern, from_bigrams$ngrams, value = FALSE)
        #print(result_idx)
        result <- from_bigrams[result_idx, ]
        #print(result)
        
        # REMOVE ENGLISH STOP WORDS FROM BEST RESULTS
        result <- remove_stop_words_from_ngrams(result)
...
look_for_ngram_from_ngrams <- function(bigram, from_trigrams) {
        #print(head(from_trigrams))
        pattern <- paste0("^", tolower(bigram), " ")
        result_idx <- grep(pattern, from_trigrams$ngrams, value = FALSE)
        #print(result_idx)
        result <- from_trigrams[result_idx, ]
        #print(result)
...
get_first_n_words <- function(df, n) {
        ul = unlist(strsplit(df, split = "\\s+"))[1:n]
        return(paste(ul, collapse = " "))
}

get_first_3_words <- function(df) {
        return(get_first_n_words(df, 3))
}

get_first_2_words <- function(df) {
        return(get_first_n_words(df, 2))
}

get_first_1_words <- function(df) {
        return(get_first_n_words(df, 1))
}


build_prediction_from_ngrams <- function(ngrams) {
        ## 
        # ngrams data sets have the following forms:
        # ngrams freq prop
        # the purpose of this function is to make them this form:
        # (n-1grams) next_word freq
#        print(head(ngrams))
#        print(head(stri_extract_last_words(ngrams$ngrams)))
#        print(head(stri_count_words(ngrams$ngrams)))
        
#        print(head(stri_split_fixed(ngrams$ngrams, " ", n = 3, tokens_only = TRUE, omit_empty = TRUE)))
#        first_words_from_ngrams <- stri_split_coll(ngrams$ngrams, " ", n = 1, tokens_only = TRUE, omit_empty = TRUE)
#        words_from_ngrams <- stri_split_coll(ngrams$ngrams, " ", n = 3, tokens_only = TRUE, omit_empty = TRUE)
#        first_n_words_from_ngrams <- sapply(X = ngrams[1:10, "ngrams"],
#                                            FUN = function(x) {
#                                                    tokens <- tokenize_words(x = x)
                                                    #first_n <- head(x = tokens[[1]], n = (length(tokens[[1]]) - 1))
                                                    #print("first_n: ")
                                                    #print(first_n)
#                                                    result <- ""
#                                                    for (i in 1:(length(tokens[[1]]) - 1)) {
#                                                            if (!(is.null(result)) & (result == ""))
#                                                                    result <- tokens[[1]][i]
#                                                            else
#                                                                    result <- paste(result, tokens[[1]][i], sep = " ")
#                                                    }
#                                                    result
#                                            })
        #first_n_words_from_ngrams <- data.frame()
        #last_words_from_ngrams <- data.frame()
        prediction_from_ngrams <- data.frame()
        tokens <- tokenize_words(ngrams$ngrams)
#        print("tokens:")
#        print(head(tokens))
#        print("length(tokens):")
#        print(length(tokens))
...
#                print("tokens[[i]]:")
#                print(head(tokens[[i]]))
                last_word <- tokens[[i]][length(tokens[[i]])]
                first_n_words <- ""
                for (k in 1:(length(tokens[[i]]) - 1)) {
                        if (!(is.null(first_n_words)) & (first_n_words == ""))
                                first_n_words <- tokens[[i]][k]
                        else
                                first_n_words <- paste(first_n_words, tokens[[i]][k], sep = " ")
                }
                #first_n_words_from_ngrams <- rbind(first_n_words_from_ngrams,
                #                                   data.frame(ngrams = first_n_words))
                #last_words_from_ngrams <- rbind(last_words_from_ngrams, last_word)
                prediction_from_ngrams <- rbind(prediction_from_ngrams,
                                                data.frame(ngrams = first_n_words,
                                                           next_word = last_word))
        }
#        print(head(prediction_from_ngrams))
        prediction_from_ngrams <- cbind(prediction_from_ngrams, ngrams$freq)
#        print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
#        print(head(first_n_words_from_ngrams))
#        print(head(last_words_from_ngrams))
#        print(head(prediction_from_ngrams))
#        last_words_from_ngrams <- stri_extract_last_words(ngrams$ngrams)
#        prediction_from_ngrams <- NULL
#        prediction_from_ngrams <- data.frame(ngrams = ngrams$ngrams,
#                                             next_word = last_words_from_ngrams,
#                                             freq = ngrams$freq)
#        for (i in 1:length(words_from_ngrams)) {
#                prediction_from_ngrams <- rbind(prediction_from_ngrams,
#                                                data.frame(ngrams = paste(words_from_ngrams[[i]][1],
#                                                                          words_from_ngrams[[i]][2]),
#                                                           next_word = words_from_ngrams[[i]][3],
#                                                           freq = ngrams[, "freq"][[i]]))
#        }
        return(prediction_from_ngrams)
}
```

```{r text_prediction_create_and_store_ngrams}
print("###############################################################")
print("Start building quadrigrams data frame")
#print("colnames(train4Gram)")
#print(colnames(train4Gram))
print(paste0("nrow(train4Gram): ", nrow(train4Gram)))
quadrigrams <- train4Gram[train4Gram[, "freq"] != 1, ]
print(paste0("nrow(train4Gram): ", nrow(quadrigrams)))
quadrigrams[, "next_word"] <- stri_extract_last_words(quadrigrams$ngrams)
quadrigrams[, "ngrams"] <- as.character(quadrigrams[, "ngrams"])
quadrigrams[, "ngrams"] <- unlist(lapply(quadrigrams[, "ngrams"], get_first_3_words))

print("Quadrigrams done.")
print(head(quadrigrams))
# Save dataframe 'quadrigrams' to rda file
#save(quadrigrams, file = "quadrigrams.rda")

print("Start building trigrams data frame")
#print("colnames(train3Gram)")
#print(colnames(train3Gram))
print(paste0("nrow(train3Gram): ", nrow(train3Gram)))
trigrams <- train3Gram[train3Gram[, "freq"] != 1, ]
print(paste0("nrow(train3Gram): ", nrow(trigrams)))
trigrams[, "next_word"] <- stri_extract_last_words(trigrams$ngrams)
trigrams[, "ngrams"] <- as.character(trigrams[, "ngrams"])
trigrams[, "ngrams"] <- unlist(lapply(trigrams[, "ngrams"], get_first_2_words))

print("Trigrams done.")
print(head(trigrams))
# Save dataframe 'trigrams' to rda file
#save(trigrams, file = "trigrams.rda")

print("Start building bigrams data frame")
#print("colnames(train2Gram)")
#print(colnames(train2Gram))
print(paste0("nrow(train2Gram): ", nrow(train2Gram)))
bigrams <- train2Gram[train2Gram[, "freq"] != 1, ]
print(paste0("nrow(train2Gram): ", nrow(bigrams)))
bigrams[, "next_word"] <- stri_extract_last_words(bigrams$ngrams)
bigrams[, "ngrams"] <- as.character(bigrams[, "ngrams"])
bigrams[, "ngrams"] <- unlist(lapply(bigrams[, "ngrams"], get_first_1_words))

print("Bigrams done.")
print(head(bigrams))
# Save dataframe 'bigrams' to rda file
...
ngrams <- rbind(bigrams, trigrams, quadrigrams)
# Save dataframe 'bigrams' to rda file
save(ngrams, file = "ngrams.rda")
```
